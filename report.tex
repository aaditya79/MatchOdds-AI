...
Table \ref{tab:main} reports aggregate metrics for all three methods over 132 games.
...
CoT is relatively well-calibrated in the middle probability ranges. Multi-agent debate shows severe overconfidence at high predicted probabilities.
...
The three sources with real historical data in the backtest show positive deltas: h2h (+0.010), stats (+0.007), and vector\_store (+0.006) — confirming they contribute to CoT performance. The four sources without historical snapshots (youtube, news, odds, injuries) all show negative or near-zero deltas, reflecting sample variation rather than real signal since they return no data for historical games.
...
CoT is stable across back-to-back and non-back-to-back games (delta $-0.011$), while both agent-based methods degrade substantially on back-to-back games. This is consistent with the home/away inversion failure mode being more pronounced in scheduling-complex situations. Star player absence breakdowns are not reported because the backtest has no per-day historical injury snapshots.
