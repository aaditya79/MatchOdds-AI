...
CoT separates evidence gathering from evidence interpretation. Every game gets the same six tool outputs in the same order, and the model makes a single reasoning pass over a fixed context. The single agent and multi-agent debate, by contrast, interleave reasoning with tool dispatch across many sequential LLM calls. Each call must both interpret the previous observation and decide what to request next, giving parsing errors and home/away label confusion more opportunities to corrupt the final JSON output. The inversion bug seen in manual inspection is a direct consequence of this multi-step structure.
